    ...
    if buttons[row][column]['text'] == "" and check_winner()is False: 
        if player == players [0]:
            ...
            if check_winner()is False:  
                player = players[1]
                label.config(text =("Rodada do " + players[1]))

            elif check_winner()is True:
                label.config(text=(players[0] + " é o vencedor"))
            ...
            if check_winner()is False:  
                player = players[0]
                label.config(text =("Rodada do " + players[0]))

            elif check_winner()is True:
                label.config(text=(players[1] + " é o vencedor"))
...
def new_game():
    ...
    label.config(text = "Rodada do " + player)
...
label = Label(text =  "Rodada do " + player , 
              font = ('System', 40) )
label.pack(side = "top")

reset_button = Button(text = "NOVO JOGO", 
                      font =('System', 20), 
                      command = new_game)
...
for row in range (3):
    for column in range(3):
        buttons [row][column] = Button(frame, text = "", font=('System', 40), width=5, height=2, command = lambda row=row, column=column: next_turn(row, column))
